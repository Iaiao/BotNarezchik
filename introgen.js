...
const TOKEN = process.env.YTA_TOKEN
        ...
        new Jimp(1920, 1080, async (err, image) => {
            if(err) {
                ...
                image.write("commenters.png", () => {
                    let proc = cp.spawn("ffmpeg", "-y -i intro.mp4 -loop 1 -i commenters.png -filter_complex [0:v]chromakey=#00ff00:0.3:0.1[c];[1:v]fade=in:50:20[f];[f]fade=out:90:10[f];[f][c]overlay[out] -map:v [out] -c:v png -t 3.2 intro.mov".split(" "))
